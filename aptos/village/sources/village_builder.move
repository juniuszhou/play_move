...
module overmind::village_builder {
    ...
    use aptos_framework::account::{Self, new_event_handle, SignerCapability, create_resource_account,
        create_signer_with_capability
    };
    use aptos_framework::coin::transfer;
    use aptos_framework::object::{address_from_constructor_ref, transfer_to_object, is_owner};
    ...
    */
    fun init_module(admin: &signer) {
        let (resource_account, resource_cap) = create_resource_account(admin, SEED);
        register<AptosCoin>(&resource_account);
        let collection_address = collection::create_unlimited_collection(
            &resource_account,
        ...
        let state = State {
            ...
            // Village collection address
            collection_address: object::address_from_constructor_ref(&collection_address),
            // Events
            create_village_events: account::new_event_handle<CreateVillageEvent>(admin),
            upgrade_building_events: account::new_event_handle<UpgradeBuildingEvent>(admin),
        };
    ...
    */
    public entry fun build_village(
        ...
        let state = borrow_global_mut<State>(account::create_resource_address(&@overmind, SEED));
        // let nft = collection::
        let resource_signer = create_signer_with_capability(&state.signer_capability);
        let village_name = string_utils::format2(&b"Village #{}: {}", state.village_id, name);

        let nft = token::create_named_token(&resource_signer, string::utf8(b"Village collection name"),
            description, village_name,  option::none(), string::utf8(b"Village collection URI"), );

        // object::
        // let token_data_id = aptos_token::token::cstate.village_id = state.village_id + 1;
        // object::transfer();
        simple_map::add(&mut state.villages, state.village_id,
            object::address_from_constructor_ref(&nft));
        // reate_tokendata();
        state.village_id = state.village_id + 1;
        let village_address = address_from_constructor_ref(&nft);
        let village_signer = object::generate_signer(&nft);

        object::transfer_raw(&resource_signer, village_address, address_of(village_builder));

        let village = &mut Village {
            name,
            // Village description - provided by the village creator
            description,
            // Buildings: building ID to current building level
            // Note: a village starts with all buildings at level 0
            buildings: simple_map::new(),
            // Timestamp of when building upgrade will finish and new one can start
            // Note: a new building can be upgrade if the current timestamp is greater than or equal to
            // this timestamp
            building_upgrade_unlock_timestamp_seconds: 0,
        };
        let i = 1_u8;
        let length = (vector::length(&BUILDING_MAX_LEVELS) as u8);
        while (i <= length) {
            simple_map::add(&mut village.buildings, i, 0);
            i = i + 1;
        };

        move_to<Village>(&village_signer, *village);
        emit_event(&mut state.create_village_events, CreateVillageEvent {
            village_builder: address_of(village_builder),
            // Address of the village NFT
            village_address: object::address_from_constructor_ref(&nft),
            // Timestamp (in seconds) of when the village was built
            village_creation_timestamp_seconds: now_seconds(),
        })
    ...
    */
    public fun upgrade_building(
        ...
        let state = borrow_global_mut<State>(account::create_resource_address(&@overmind, SEED));
        assert!(simple_map::contains_key(&state.villages, &village_id), ECodeForAllErrors);


        let village_address = simple_map::borrow_mut(&mut state.villages, &village_id);
        let village = borrow_global_mut<Village>(*village_address);

        assert!(simple_map::contains_key(&village.buildings, &building_id), ECodeForAllErrors);

        let village_token_object =
            object::address_to_object<token::Token>(*village_address);


        assert!(is_owner(village_token_object, address_of(account)), ECodeForAllErrors);

        let (r0, r1) = get_building_requirements(building_id);
        if (r0 != 0) {
            let required_level = simple_map::borrow(&mut village.buildings, &r0);
            assert!(*required_level >= r1, ECodeForAllErrors);
        };


        let current_level = simple_map::borrow_mut(&mut village.buildings, &building_id);

        assert!(*current_level < get_building_max_level(building_id), ECodeForAllErrors);

        let duration = get_building_level_upgrade_duration(building_id);
        assert!(village.building_upgrade_unlock_timestamp_seconds <= now_seconds(), ECodeForAllErrors);

        let cost = get_building_upgrade_cost(building_id);
        let balance = coin::balance<AptosCoin>(address_of(account));
        assert!(balance >= cost, ECodeForAllErrors);

        coin::transfer<AptosCoin>(account, account::create_resource_address(&@overmind, SEED), cost);

        village.building_upgrade_unlock_timestamp_seconds = now_seconds() + duration;
        // #[test]
        // debug::print(&village.building_upgrade_unlock_timestamp_seconds);
        *current_level = *current_level + 1;
        // simple_map::add(&mut village.buildings, building_id, *current_level + 1);

        emit_event(&mut state.upgrade_building_events, UpgradeBuildingEvent {
            village_address: *village_address,
            // Id of the building that was upgraded
            building_id,
            // New level of the building
            new_building_level: *current_level,
            // Timestamp (in seconds) of when the building was upgraded
            building_upgrade_timestamp_seconds: now_seconds(),
        });
    ...
    */
    ...
    public fun get_building_max_level(
        ...
        *vector::borrow(vector::borrow(&BUILDING_MAX_LEVELS, (building_id as u64) - 1), 1)
    }
    ...
    */
    ...
    public fun get_building_requirements(
        ...
        // if (!vector::any(&BUILDING_REQUIREMENTS, |element| *vector::borrow(element, 0) == building_id)) {
        //     return (0, 0)
        // };

        let (found, index) = vector::find(&BUILDING_REQUIREMENTS, |element| *vector::borrow(element, 0) == building_id);

        if (found) {
            let v = vector::borrow(&BUILDING_REQUIREMENTS, index);
            (*vector::borrow(v, 1), *vector::borrow(v, 2))
        } else {
            (0, 0)
        }
    ...
    */
    ...
    public fun get_building_level_upgrade_duration(
        ...
        *vector::borrow(vector::borrow(&BUILDING_UPGRADE_DURATION, (building_level as u64) - 1), 1)
    }
    ...
    */
    ...
    public fun get_building_upgrade_cost(
        ...
        *vector::borrow(vector::borrow(&BUILDING_UPGRADE_COST, (building_id as u64) - 1), 1)
    }
    ...
    fun test_build_village_success(
        ...
        let village_name = string_utils::format2(&b"Village #{}: {}",
            1, string::utf8(b"village_name"));
        ...
        assert!(
            token::description(village_token_object) == 
                // string::utf8(b"Village collection description"),
                string::utf8(b"village_description"),
            0
        );
        assert!(
    ...
    fun test_build_village_success_multiple_villages_same_user(
        ...
        assert!(
            token::description(village_token_object_2) == 
                // string::utf8(b"Village collection description"),
            string::utf8(b"village_description_2"),
            0
        );
        assert!(
    ...
    fun test_upgrade_building_success_multiple_buildings(
        ...
        timestamp::fast_forward_seconds(get_building_level_upgrade_duration(4));
        ...
        assert!(
            village.building_upgrade_unlock_timestamp_seconds == 
                timestamp::now_seconds() + get_building_level_upgrade_duration(9),
            6
        );
