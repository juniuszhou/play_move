module overmind::governance {
    ...
    use aptos_framework::object::{Self, Object, generate_signer};
    ...
    */
    fun init_module(admin: &signer) {
        ...
        let (resource_account, signer_capability) = account::create_resource_account(admin, SEED);
        let resource_account_address = address_of(&resource_account);
        // TODO: Register with the aptos_framework::voting module using the GovernanceProposalType and resource account
        voting::register<GovernanceProposalType>(&resource_account);
        ...
        // aptos_token::token::create_collection(&resource_account,
        //     STARTING_GOVERNANCE_TOKEN_DESCRIPTION,
        //     GOVERNANCE_TOKEN_COLLECTION_NAME,
        //     0,
        //     STARTING_GOVERNANCE_TOKEN_URI);
        ...
        mint_governance_token(&resource_account, string::utf8(STARTING_GOVERNANCE_TOKEN_URI),
            @overmind, ADMIN_STARTING_VOTING_POWER, ADMIN_STARTING_PROPOSAL_ABILITY);
        ...
        let responsibility = GovernanceResponsibility{
            signer_caps: simple_map::create()
        };

        simple_map::add(&mut responsibility.signer_caps, GovernanceResponsibilityKey {
        ...
        let execution_hashs = ApprovedExecutionHashes {
            hashes: simple_map::create()
        };

        // TODO: Create the GovernanceEvents object
        // let handler = account::new_event_handle<VoteEvent>(&resource_account);
        let events = GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(&resource_account),
            vote_events: account::new_event_handle<VoteEvent>(&resource_account),
            updated_governance_config_events: account::new_event_handle<UpdateGovernanceConfigEvent>(&resource_account),
            update_governance_responsibility_events: account::new_event_handle<UpdateGovernanceResponsibilityEvent>(&resource_account),
            add_approved_execution_hash_events: account::new_event_handle<AddApprovedExecutionHashEvent>(&resource_account),
            resolve_proposal_events: account::new_event_handle<ResolveProposalEvent>(&resource_account)
        };
        
        // TODO: Move the 4 new global objects to the resource account
        move_to(&resource_account, config);
        move_to(&resource_account, responsibility);
        move_to(&resource_account, execution_hashs);
    ...
    public fun mint_governance_token(
        ...
        let description = generate_governance_token_description(address_of(creator));
        // TODO: Generate the name for this new governance token
        //
        // HINT: use the generate_governance_token_name function below
        let name = generate_governance_token_name(address_of(creator));

        // TODO: Create a new named token in the governance token collection with no royalty
        let named_token = token::create_named_token(creator, string::utf8(GOVERNANCE_TOKEN_COLLECTION_NAME), description, name, option::none(), uri);
        // token::
        // named_token
        // TODO: Transfer the token to the new member address and disable ungated transfer to enable
        //       soul bound functionality
        // object::transfer(&generate_signer(&named_token), );


        // move_to(soul_bound_to, token::mint(creator, string::utf8(GOVERNANCE_TOKEN_COLLECTION_NAME), soul_bound_to, voting_power, can_propose));
    ...
    */
    ...
    inline fun check_if_address_is_an_object(object_address: address) {
        ...
        assert!(exists<GovernanceConfig>(object_address), EAddressIsNotAnObject);
    }
    ...
    inline fun check_if_account_is_resource_account_address(account_address: address) {
        ...
        assert!(exists<GovernanceConfig>(account_address), EAccountIsNotResourceAccount);
    ...
    fun test_init_module_success(
        ...
        let a = b"++++++";
        std::debug::print(&a);
        std::debug::print(&collection::count<Collection>(governance_token_collection));
        assert!(
