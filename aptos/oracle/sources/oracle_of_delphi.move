...
module overmind::price_oracle {
    ...
    */
    fun init_module(admin: &signer) { 
        ...
        let (resource_signer, resource_cap) = create_resource_account(admin, SEED);

        // TODO: Create the module's State resource and move it to the resource account
        let state = State {
            signer_cap: resource_cap,
            // events
            price_feed_updated_event: account::new_event_handle<PriceFeedUpdatedEvent>(&resource_signer)
        };
        move_to(&resource_signer, state);

        // TODO: Create the module's PriceBoard resource and move it to the resource account
        let price_board = PriceBoard {
            prices: table::new<String, PriceFeed>(),
        };
        move_to(&resource_signer, price_board);
    }
    ...
    */
    public entry fun update_price_feed(
        ...
        assert!(address_of(admin) == @overmind, ErrorCodeForAllAborts);
        let now = now_seconds();
        let price_borard = borrow_global_mut<PriceBoard>(account::create_resource_address(&@overmind, SEED));
        if (table::contains(&price_borard.prices, pair) == false) {
            let price_feed = PriceFeed {
                latest_attestation_timestamp_seconds: now,
                // the id of the coin pair - e.g. "APT/USDT"
                pair,
                // the latest, attested price of the pair
                price: Price {
            ...
            table::add(&mut price_borard.prices, pair, price_feed);
        } else {
            let price_feed: &mut PriceFeed = table::borrow_mut(&mut price_borard.prices, pair);

            price_feed.latest_attestation_timestamp_seconds = now;
            price_feed.price.price = price;
            price_feed.price.confidence = confidence;
        };

        let state = borrow_global_mut<State>(account::create_resource_address(&@overmind, SEED));
        emit_event(&mut state.price_feed_updated_event, PriceFeedUpdatedEvent {
            pair,
            // the latest, attested price of the pair
            price: Price {
                price,
    ...
    */
    public fun get_price(pair: String): Price acquires PriceBoard {
        let price_borard = borrow_global<PriceBoard>(account::create_resource_address(&@overmind, SEED));
        assert!(table::contains(&price_borard.prices, pair), ErrorCodeForAllAborts);

        let price_feed: &PriceFeed = table::borrow(&price_borard.prices, pair);
        assert!(price_feed.latest_attestation_timestamp_seconds + 60 * 60 * 2 >= timestamp::now_seconds(), ErrorCodeForAllAborts);
        price_feed.price
    }
    ...
    */
    ...
    fun test_get_price_no_older_than_success_multiple_prices_within_duration(
        ...
        print(&admin_address);
        // print("+++++++++++");
        print(&resource_account_address);
